...
const moodMapping: Record<string, MoodResult> = {
  happy: { mood: "Happy", confidence: 0, emoji: "😊", color: "from-yellow-400 to-orange-500", genre: "Pop", dayDescription: "Masz świetny dzień! Energia pozytywna promieniuje z Ciebie!" },
  sad: { mood: "Melancholic", confidence: 0, emoji: "😢", color: "from-blue-400 to-indigo-500", genre: "R&B", dayDescription: "Wygląda na trudniejszy dzień... Muzyka poprawi Ci nastrój!" },
  angry: { mood: "Intense", confidence: 0, emoji: "😤", color: "from-red-500 to-orange-600", genre: "Rock", dayDescription: "Czujesz się intensywnie! Czas na muzykę, która to uwolni!" },
  fearful: { mood: "Anxious", confidence: 0, emoji: "😰", color: "from-purple-400 to-pink-500", genre: "Trance", dayDescription: "Stresujący dzień? Uspokajające dźwięki pomogą!" },
  disgusted: { mood: "Rebellious", confidence: 0, emoji: "😒", color: "from-green-500 to-teal-500", genre: "Punk", dayDescription: "Buntowniczy nastrój! Rock i punk dla Ciebie!" },
  surprised: { mood: "Excited", confidence: 0, emoji: "😮", color: "from-pink-400 to-rose-500", genre: "EDM", dayDescription: "Pełen ekscytacji dzień! Czas na energetyczną muzykę!" },
  neutral: { mood: "Relaxed", confidence: 0, emoji: "😌", color: "from-cyan-400 to-blue-500", genre: "House", dayDescription: "Spokojny, zrelaksowany dzień. Idealna pora na chill!" },
  energetic: { mood: "Energetic", confidence: 0, emoji: "⚡", color: "from-orange-400 to-red-500", genre: "EDM", dayDescription: "WOW! Mega energetyczny dzień! Czas rozkręcić imprezę!" },
  romantic: { mood: "Romantic", confidence: 0, emoji: "💕", color: "from-pink-300 to-rose-400", genre: "R&B", dayDescription: "Romantyczny nastrój... Czas na piękne melodie!" },
  focused: { mood: "Focused", confidence: 0, emoji: "🎯", color: "from-indigo-400 to-purple-500", genre: "House", dayDescription: "Skupiony i gotowy do działania! Muzyka pomoże!" },
};
...
export const QuickMoodDetector = ({ isOpen, onClose }: QuickMoodDetectorProps) => {
  ...
  const fetchDeepAnalysis = useCallback(async (mood: MoodResult, emotion: string) => {
    ...
    setAnalysisStep("🧠 Profesor AI analizuje Twój stan...");
    ...
    try {
      ...
      toast.error("Nie udało się przeprowadzić głębokiej analizy AI.");
    } finally {
  ...
  const startAnalysis = useCallback(async () => {
    ...
    progressIntervalRef.current = setInterval(() => {
      ...
      if (progress > 10 && progress < 30) setAnalysisStep("Wykrywanie twarzy...");
      else if (progress > 30 && progress < 60) setAnalysisStep("Analiza wyrazu twarzy...");
      else if (progress > 60 && progress < 85) setAnalysisStep("Rozpoznawanie emocji...");
      else if (progress > 85) setAnalysisStep("Finalizacja wyników...");
    }, 75);
    ...
    try {
      ...
      // Play 5 tracks based on analysis
  ...
  }, [isModelLoaded, detectWithSampling, fetchDeepAnalysis, playMoodPlaylist]);
  ...
  const riskColor = deepAnalysis?.riskLevel === "wysoki" ? "text-red-400" : deepAnalysis?.riskLevel === "średni" ? "text-yellow-400" : "text-green-400";
      ...
      >
        ...
        <div className="flex items-center justify-between p-3 pt-6 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">AI Profesor Psychologii</h3>
              <p className="text-[10px] text-white/50">
                {isDeepAnalyzing ? "Głęboka analiza psychiatryczna..." : isLoadingModel ? "Ładowanie TensorFlow.js..." : isModelLoaded ? "TensorFlow.js + AI gotowy" : "face-api.js"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="relative aspect-video bg-black/50">
              ...
              {!isActive && !isLoadingModel && !currentMood && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                  ...
                  <p className="text-xs text-white/40 text-center px-4">
                    AI Profesor przeanalizuje Twoje emocje i dobierze 5 idealnych utworów
                  </p>
                  {isModelLoaded && (
              ...
              {!isActive && isLoadingModel && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                  ...
                  <p className="text-xs text-white/60 text-center px-4">Ładowanie modeli face-api.js...</p>
              ...
              {isActive && noFaceDetected && !isAnalyzing && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 flex flex-col items-center justify-center bg-black/60">
                  ...
                  <p className="text-white/60 text-xs text-center px-6 mt-1">Upewnij się, że twarz jest dobrze oświetlona</p>
                  <Button onClick={startAnalysis} size="sm" className="mt-3 groove-gradient-bg">Spróbuj ponownie</Button>
                </motion.div>
              )}
            </div>
          )}
          ...
          {isDeepAnalyzing && currentMood && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 flex flex-col items-center gap-3">
              ...
              <div className="w-full space-y-1.5 mt-2">
                {["Analiza mikroekspresji...", "Diagnoza psychiatryczna...", "Dobór muzykoterapii...", "Selekcja 5 utworów..."].map((step, i) => (
                  <motion.div
                    key={step}
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-3 space-y-3">
              ...
              <div className={`p-3 rounded-xl bg-gradient-to-r ${currentMood.color} relative overflow-hidden`}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-white/80 text-xs">{currentMood.confidence}% pewności detekcji</p>
                  </div>
                  {tracksPlaying && (
              ...
              <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="text-white font-semibold text-xs uppercase tracking-wider">Diagnoza Profesora</h4>
                </div>
              ...
              <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="text-white font-semibold text-xs uppercase tracking-wider">Stan Emocjonalny</h4>
                </div>
              ...
              <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h4 className="text-white font-semibold text-xs uppercase tracking-wider">Poziom Ryzyka</h4>
              ...
              >
                {showFullAnalysis ? "Zwiń" : "Pełna analiza psychiatryczna"}
              ...
              <AnimatePresence>
                {showFullAnalysis && (
                  ...
                  >
                    {/* Micro Expressions */}
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <h4 className="text-white font-semibold text-xs uppercase tracking-wider mb-1">🔬 Mikroekspresje</h4>
                      <p className="text-white/70 text-xs leading-relaxed">{deepAnalysis.microExpressions}</p>
                    </div>

                    {/* Psychological Insight */}
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <h4 className="text-white font-semibold text-xs uppercase tracking-wider mb-1">🧩 Wgląd Psychologiczny</h4>
                      <p className="text-white/70 text-xs leading-relaxed">{deepAnalysis.psychologicalInsight}</p>
                    </div>

                    {/* Therapeutic Advice */}
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <h4 className="text-white font-semibold text-xs uppercase tracking-wider mb-1">💊 Zalecenia Terapeutyczne</h4>
                    ...
                    <div className="p-3 rounded-xl bg-gradient-to-r from-primary/20 to-accent/20 border border-primary/20">
                      <h4 className="text-white font-semibold text-xs uppercase tracking-wider mb-1">🎵 Muzykoterapia</h4>
                      <p className="text-white/70 text-xs leading-relaxed">{deepAnalysis.musicTherapy}</p>
                    </div>

                    {/* Healing Frequency */}
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <h4 className="text-white font-semibold text-xs uppercase tracking-wider mb-1">〰️ Częstotliwość Lecznicza</h4>
              ...
              {tracksPlaying && (
                ...
                >
                  ...
                  Gram 5 utworów dobranych przez AI Profesora
                </motion.p>
              )}
            </motion.div>
          )}
          ...
          <div className="p-3 space-y-2">
            ...
            {modelError && (
              <p className="text-[10px] text-yellow-400 text-center">{modelError} - używam trybu uproszczonego</p>
            )}

            {hasPermission === false && (
              <p className="text-[10px] text-destructive text-center">Brak dostępu do kamery. Sprawdź uprawnienia przeglądarki.</p>
            )}
          </div>
        </div>
      </motion.div>
