...
// zapisuje pakiet w hub_social_queue i zwraca 503.
//
// v2 (2026-07-11): firehose newsów NIE idzie już domyślnie na Discord — Greg prosił,
// by nie zaśmiecać serwera. Discord dostaje pakiety social tylko gdy
// hub_config.discord_social_enabled = 'true'. Telegram działa jak dawniej.
...
async function sendDiscord(webhookUrl: string, title: string, content: string, link: string) {
  // Elegancki embed zamiast surowego tekstu.
...
Deno.serve(async (req) => {
  ...
  // Discord tylko na wyraźne włączenie — inaczej firehose newsów zaśmieca elegancki serwer.
  if (cfg["discord_webhook_url"] && cfg["discord_social_enabled"] === "true") {
    const text = `${social.twitter_post || social.facebook_post || ""}`;
    channels.push({ name: "discord", send: () => sendDiscord(cfg["discord_webhook_url"], title, text, link) });
  }
  ...
  if (channels.length === 0) {
    await db.from("hub_log").insert({
      ...
      message: `Pakiet "${title}" zakolejkowany — brak aktywnych kanałów (telegram pusty, discord wyłączony)`,
