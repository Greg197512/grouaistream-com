...
// Okładki AI dla GrouAI Studio. Kolejność: (1) HF FLUX.1-schnell (DARMOWE, hf_token),
// (2) FLUX 1.1 Pro na Replicate (płatne, top jakość), (3) Pollinations (darmowe awaryjnie).
// Przełącznik hub_config.cover_provider="flux" wymusza Replicate jako pierwszy (max jakość).
// Zapis do bucketa `acestep` jako {id}-cover.jpg. Auth: JWT użytkownika LIVE (bvstv).
...
Deno.serve(async (req) => {
  ...
  const preferProvider = cfgGet("cover_provider"); // "flux" → wymusza Replicate jako pierwszy

  // ===== 1) HF FLUX.1-schnell (DARMOWE) — domyślnie pierwszy, chyba że cover_provider=flux =====
  const hfToken = cfgGet("hf_token");
  const hfModel = cfgGet("hf_cover_model") || "black-forest-labs/FLUX.1-schnell";
  if (hfToken && preferProvider !== "flux") {
    try {
      ...
      if (r.ok && ct.startsWith("image")) {
        ...
        if (bytes.length > 5000) {
          const url = await store(bytes, ct || "image/jpeg");
          return json({ cover_url: url, id, source: "hf-flux-schnell" });
        }
      }
      console.warn("[studio-cover] HF failed, fallback:", r.status, ct);
    } catch (e) {
      console.warn("[studio-cover] HF error, fallback:", String(e).slice(0, 120));
    }
  }

  // ===== 2) FLUX na Replicate (najwyższa jakość, płatne) =====
  ...
  if (repToken) {
    try {
      ...
      console.warn("[studio-cover] FLUX failed, fallback:", data?.error || rel.status);
    } catch (e) {
      console.warn("[studio-cover] FLUX error, fallback:", String(e).slice(0, 120));
    }
  }

  // ===== 3) Awaryjnie: Pollinations (darmowe) =====
  try {
